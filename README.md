...
\#\# Legend (semantics)  
\- 🟧 \*\*Domain Event\*\* (past tense, fact that happened)  
\- 🔵 \*\*Command\*\* (intention to change state)  
\- 🟨 \*\*Aggregate\*\* (consistency boundary)  
\- 🟣 \*\*Policy / Saga\*\* (reactive process)  
\- 🟩 \*\*Read Model / Projection\*\* (query-optimized view)  
\- ⚫ \*\*External System\*\* (PSP, e-Invoice, Notifier, IoT Broker)  
\- 🟪 \*\*Rule/Invariant\*\* (business constraint)
...
\- 🟪 PasswordPolicy (length/complexity/expiry)  
\- 🟪 RateLimitLogin (N failed ⇒ temporary lock)  
\- 🟪 RBAC (only roles with permission can access protected resources)
...
\#\#\# Edge cases  
\- Verification token expired ⇒ re-issue token; lock after 3 invalid tries.    
\- PasswordReset ⇒ \*\*invalidate all active sessions\*\*.
...
\- 🟪 MinimalData (name \+ email/id required)  
\- 🟪 Privacy (DeleteAccount ⇒ anonymize PII)

\#\#\# Read models (🟩)  
\- \*\*ProfileView\*\*, \*\*UserPreferences\*\*

\#\#\# Happy path  
🔵 CreateProfile → 🟧 ProfileCreated → 🔵 ConfigurePreferences → 🟧 PreferencesConfigured

\#\#\# Edge cases  
\- Invalid identity document ⇒ reject with reason and fix hints.
...
\- 🟪 PSP Idempotency (idempotency-key per intent)  
\- 🟪 Reconciliation (amount/currency must match order)
...
\#\#\# Edge cases  
\- Authorized but not captured (timeout) ⇒ cancel and retry.    
\- Duplicate webhook ⇒ ignore via idempotency.
...
\- 🟪 TimeWindow (5 min aggregations for kWh)  
\- 🟪 De-dup (same reading by deviceId+timestamp is ignored)  
...
\#\#\# Happy path  
(⚫ IoT Broker) → 🟧 ReadingReceived → 🟧 ReadingNormalized → 🟧 AnomalyDetected → 🟧 AlertRaised → (⚫ Notifier) 🟧 NotificationDelivered → 🔵 AcknowledgeAlert → 🟧 AlertAcknowledged → 🔵 CloseAlert → 🟧 AlertClosed
...
\- 🟪 ReportSLA (\<10 s for small datasets; else async job)  
\- 🟪 ReadConsistency (reports query \*\*Read DB\*\* only)
...
\#\#\# Happy path  
🔵 GenerateReport → 🟧 KPIsCalculated → 🟧 ReportGenerated → 🔵 DownloadReport

\#\#\# Edge cases  
\- Large dataset ⇒ enqueue async job; notify on completion.
...
1\) \*\*Discovery/Conversion (Landing – EP03)\*\*    
   🔵 ViewServices/Projects/IoTBenefits → Registration/Login (IAM)  
2\) \*\*Project & preferences\*\*    
   🔵 CreateProfile → 🟧 ProfileCreated    
   🔵 ConfigurePreferences → 🟧 PreferencesConfigured  
3\) \*\*Execution & monitoring\*\*    
   (⚫ IoT) 🟧 ReadingReceived → 🟧 AlertRaised (if applicable) → Notifier    
   🔵 UploadEvidence/Document → 🟧 DocumentUploaded    
   🔵 ReportIncident → 🟧 IncidentLogged  

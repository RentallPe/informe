...
| 51    | US51          | Cambiar idioma                    |Como usuario, quiero cambiar el idioma para poder ver el contenido en mi idioma preferido.                                                            | 5      |
| 52    | US52          | Ver página “No encontrada”                    | Como usuario, quiero ver un mensaje de “Página no encontrada” cuando acceda a una ruta inválida para saber que no hay contenido disponible en esa ruta.                                                           | 5      |
| 53    | US53          | Registro de usuario                     | Como desarrollador, quiero implementar un endpoint para registrar nuevos usuarios, para que la aplicación pueda gestionar la creación de cuentas de forma segura.                                                           | 5      |
| 54    | US54          | Autenticación de usuario                     | Como desarrollador, quiero implementar un endpoint para autenticar usuarios, para que solo usuarios válidos puedan acceder a funcionalidades protegidas.                                                           | 5      |
| 55    | US55          | Creación de Espacio                    | Como desarrollador, quiero implementar el endpoint POST /api/v1/space, para que se puedan registrar nuevos espacios con todos sus atributos requeridos en el sistema.                                                           |8    |
| 56    | US56          | Consulta de Espacios                   | Como desarrollador, quiero implementar los endpoints GET para listar (/api/v1/space) y obtener por ID (/api/v1/space/{id}), para que otros servicios puedan recuperar la información de los espacios.                                                     | 5      |
| 57    | US57          | Actualización de Espacio                   | Como desarrollador, quiero implementar el endpoint PUT /api/v1/space/{id}, para que sea posible modificar los atributos de un Espacio existente de forma controlada.                                                    | 5      |
| 58    | US58          | Eliminación de Espacio                   | Como desarrollador, quiero implementar el endpoint DELETE /api/v1/space/{id}, para que se puedan eliminar definitivamente los registros de Espacios cuando ya no sean necesarios.                                                    | 5      |
| 59    | US59          | Creación de Pago                   | Como desarrollador, quiero implementar el endpoint POST /api/v1/payments para registrar un nuevo pago en estado inicial (PENDING), asegurando la integridad de los datos.                                                    | 8      |
| 60    | US60          | Consulta de Pagos                   | Como desarrollador, quiero implementar los endpoints GET para consultar pagos por ID (/api/v1/payments/{id}) y por criterios de búsqueda (/api/v1/payments), para exponer la información de manera eficiente.                                                   | 5      |
| 61    | US61          | Flujo de Iniciación y Confirmación                   | Como desarrollador, quiero implementar los endpoints transicionales POST /api/v1/payments/{id}/initiate y /api/v1/payments/{id}/confirm, para gestionar la iniciación y el éxito de una transacción de pago.                                                  | 8     |
...
| 63    | US63          | Registro de Proyecto de Monitoreo                   | Como desarrollador, quiero implementar el endpoint POST /api/v1/monitoring/projects para registrar un nuevo proyecto en el sistema de monitorización, garantizando la unicidad y estructura de los datos.                                                 | 8      |
| 64    | US64          | Consulta de Proyecto por ID                   | Como desarrollador, quiero implementar el endpoint GET /api/v1/monitoring/projects/{id} para consultar los detalles de un proyecto específico, permitiendo a otros servicios acceder a su información de configuración.                                                 | 8      |
...
| 66    | US66          | Creación de Tarea de Monitoreo                  | Como desarrollador, quiero implementar el endpoint POST /api/v2/monitoring/tasks para registrar una nueva tarea o configuración de monitoreo, asegurando la correcta parametrización y validación.                                                 | 5      |
| 67    | US67          | Consulta de Tarea por ID                 | Como desarrollador, quiero implementar el endpoint GET /api/v2/monitoring/tasks/{id} para consultar la configuración y el estado de ejecución de una tarea específica, permitiendo el debugging y la auditoría.                                                 | 5      |
| 68    | US68          | Registro de Dispositivo IoT                 | Como desarrollador, quiero implementar el endpoint POST /api/v1/monitoring/io-t-devices para registrar un nuevo dispositivo IoT, garantizando que su ID sea único y esté asociado a un proyecto válido.                                                 | 5      |
| 69    | US69          | Consulta de Dispositivos por Proyecto | Como desarrollador, quiero implementar el endpoint GET /api/v1/monitoring/io-t-devices/project/{projectId} para consultar todos los dispositivos asociados a un proyecto de monitoreo específico, facilitando la gestión y el aprovisionamiento.              | 5      |
| 70    | US70          | Consulta de Incidentes por Proyecto | Como desarrollador, quiero implementar el endpoint GET /api/v1/monitoring/incidents/project/{projectId} para recuperar la lista de incidentes activos o históricos asociados a un proyecto específico.              | 5      |
| 71    | US71          | Reconocimiento (Acknowledge) de Incidente | Como desarrollador, quiero implementar el endpoint PATCH /api/v1/monitoring/incidents/{id}/acknowledge para cambiar el estado de un incidente a "reconocido" (acknowledged), lo cual detiene las notificaciones recurrentes.              | 5      |
| 72    | US72          | Consulta de Notificaciones por Proyecto | Como desarrollador, quiero implementar el endpoint GET /api/v1/monitoring/notifications/project/{projectId} para recuperar la lista de notificaciones generadas para un proyecto específico.              | 5      |
